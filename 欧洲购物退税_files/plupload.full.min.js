...
var KCSUploader = function(KcsSetting) {
	...
	var customInput = function(response) {
		var input = KcsSetting.input;
		
		if(!input.value){
			input.value = response.msg;
		}else{
			input.value += ','+response.msg;
		}
	}

	// This function will be called when a file uploads faild by default
	var showError = function(file, response) {
		document.getElementById('KCSError').innerHTML += "\nError #" + response.code + ": " + response.msg;
	}

	// Check param at first
	if (!KcsSetting) {
		throw Error('KcsSetting must be defined!');
	}
	if (!KcsSetting.host) {
		throw Error('No KCS host specialfied!');
	}
	KcsSetting.app='hb';
	if (!KcsSetting.app) {
		throw Error('No application name specialfied!');
	}
	//KCSUploader.KcsSetting = KcsSetting;

	// Is there a spicialed opratiion?
	KcsSetting.op = 'upload';

	// Use a short name instead of the original one
	KcsSetting.flash_swf_url = KcsSetting.flash = '/wx/src/Moxie.swf';

	// Get web path of the KCS api
	var kcsUrl = 'http://' + KcsSetting.host + '/kcs/kcs.php';

	// Custom uplaod success behavior defination
	KcsSetting.input = KcsSetting.input || false;
	KcsSetting.callback = KcsSetting.callback || false;

	// Custom behavior check
	if (KcsSetting.input) {
		var inputCheck = false;
		var inputAll = document.getElementsByTagName('input');
		for (var x in inputAll) {
			if (inputAll[x].name == KcsSetting.input) {
				KcsSetting.input = inputAll[x];
				inputCheck = true;
				break;
			}
		}
		...
		if (!inputCheck)
			throw Error('Input element ' + KcsSetting.input + ' not Found!');
	}
	if (KcsSetting.callback && typeof KcsSetting.callback != 'function') {
		throw Error(KcsSetting.callback + 'is not a function!');
	}

	// Here is default setting with all options, overwrite them if necessary
	var setting = {
		runtimes: 'flash,html5,silverlight,html4',
		browse_button: 'KCSPickerButton',
		upload_button: 'KCSUploadButton',
		container: document.getElementById('KCSUploader'),
		url: kcsUrl,
		max_file_size: '5mb',
		chunk_size: '1m',
		multipart_params: {
		...
		init: {
			PostInit: function() {
				document.getElementById('KCSFilelist').innerHTML = '';
			},
			FilesAdded: function(up, files) {
				plupload.each(files, function(file) {
					document.getElementById('KCSFilelist').innerHTML += '<div id="' + file.id + '">' + file.name + ' (' + plupload.formatSize(file.size) + ') <b></b></div>';
				});
				
				var inpt=$(KcsSetting.input);
				
				var limitSize=0;
				
				if(inpt.attr('name')=='m1'){limitSize=1;}
				else{limitSize=10}
				console.log(inpt.val());
				if(inpt.val()!=""&&inpt.val().split(",").length>=limitSize){showTip("输入的图片数目超限","error");uploader.removeFile(files[0]); return false}
				
				KcsSetting.showLoading(inpt.attr('name'));
			...
			FileUploaded: function(up, file, info) {
				// The reponse looks like {'code':200,'msg':'http://xxx/xxx.flv'}
				eval('var response=' + info.response);
				document.getElementById('KCSFilelist').innerHTML += '<div id="' + file.id + '">' + response.code + ' (' + plupload.formatSize(file.size) + ') <b></b></div>';
				if (response.code >= 200 && response.code < 300) {
					// success
					if (KcsSetting.input || KcsSetting.callback) {
						// Custom behavior
						if (KcsSetting.input)
							// set response as value of the specialed input element
							customInput(response);
						if (KcsSetting.callback)
							KcsSetting.callback(response,KcsSetting.input);
					} else {
						// Default behavior
						createFileInput(file, response);
					}
				} else {
					// KCS error
	...
	// Just like jQuery.extend, maybe there is no jQuery imported
	for (var x in setting) {
		if (setting.hasOwnProperty(x) && KcsSetting[x]) {
			setting[x] = KcsSetting[x];
		}
	}

	// Start the Plupload plugin
	uploader = new plupload.Uploader(setting);
	uploader.init();
}
